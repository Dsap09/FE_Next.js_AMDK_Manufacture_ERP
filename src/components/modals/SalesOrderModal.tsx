...
export default function SalesOrderModal({ isOpen, onClose, onSave, initialData }: Props) {
    ...
    const [formData, setFormData] = useState({
        ...
        notes: "",
        status: "approved", // Default based on backend
    });
            ...
            if (initialData) {
                ...
                if (initialData.items && initialData.items.length > 0) {
                    setItems(initialData.items.map((it: any) => ({
                        ...
                        qty: it.qty_pesanan, // Property is qty_pesanan in backend DB / model, but API wants qty
                        price: it.price
                    })));
                } else {
    ...
    const handleSubmit = () => {
        ...
        // Items are only required for store, not for update header
        if (!initialData) {
        ...
        <div className="fixed inset-0 z-[999] flex items-center justify-center bg-black/50 p-4">
            <div className="w-full max-w-4xl max-h-[90vh] overflow-y-auto rounded-2xl bg-white p-6 shadow-xl dark:bg-gray-900">
                <h3 className="mb-6 text-xl font-bold text-gray-800 dark:text-white">
                    {initialData ? "Edit Header SPK" : "Buat Surat Pesanan Konsumen (SPK)"}
                </h3>
                ...
                <div className="space-y-4">
                    ...
                    {/* Items - Hanya tampil saat Create, karena Update API hanya Header */}
                    {!initialData && (
